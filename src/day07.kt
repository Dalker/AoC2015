...
class Circuit(val logging: Boolean = false) {
    ...
    fun compute(wire: String): UInt {
        ...
        val result = when {
            assignPattern.matches(rule) -> valueOf(rule)
            notPattern.matches(rule) ->
                /* circumvent Kotlin not handling 16-bit bitwise operations... :-( */
                notPattern.find(rule)!!.groupValues.let {
                    (65536u + valueOf(it[1]).inv() - 4294967296u).toUInt()
                }
            andPattern.matches(rule) ->
                andPattern.find(rule)!!.groupValues.let {
                    valueOf(it[1]) and valueOf(it[2])
                }
            orPattern.matches(rule) ->
                orPattern.find(rule)!!.groupValues.let {
                    valueOf(it[1]) or valueOf(it[2])
                }
            lshiftPattern.matches(rule) ->
                lshiftPattern.find(rule)!!.groupValues.let {
                    valueOf(it[1]) shl it[2].toInt()
                }
            rshiftPattern.matches(rule) ->
                rshiftPattern.find(rule)!!.groupValues.let {
                    valueOf(it[1]) shr it[2].toInt()
                }
            else -> 0u.also {
